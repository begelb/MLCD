...
if __name__=='__main__':   
        ...
        index_pts_in_domain = np.where(np.linalg.norm(X1[-1], axis=1)<=max(np.linalg.norm(domain, axis=1)))
        index_pts_out_domain = np.where(np.linalg.norm(X1[-1], axis=1)>max(np.linalg.norm(domain, axis=1)))
        ...
        # plt.title("Hausdorff Distance Between Iterations")
        
        diag = compute_persistance(lifted_pts_in_domain[:,:-1]) 
        gudhi.plot_persistence_diagram(diag)
        plt.title(f'PD without Marcio norm with {num_of_pts} pts')
        plt.show()
