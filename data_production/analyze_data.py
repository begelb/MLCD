...
def train_classifier(labeled_pts):
    ...
    model = MLPClassifier(hidden_layer_sizes=(
        10, 10), activation='relu', learning_rate_init=0.01, max_iter=10000, random_state=42)
...
if __name__ == "__main__":
    ...
    if dim<3:
        ...
        plot_data.plot2dNNpred(model, grid_pts, labels_grid, path, system, title="Prediction on Grid from Neural Network")        
        plot_data.plotNNpred(model, grid_pts, path_out, title="Points")
    
    c0=model.predict_proba(grid_pts)[:,1]       
    plt.figure()
    plt.scatter(grid_pts[:,0],grid_pts[:,1], c=c0, cmap='coolwarm')
